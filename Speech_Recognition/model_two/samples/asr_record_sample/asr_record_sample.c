...
int my_reg(const char* str, const char* pattern){
    int status;
	regmatch_t pmatch[1];
...
int res_parse(char* str){
	const char* pattern1 = "<dialpre id=\"10004\">.*?</dialpre>";
	const char* pattern2 = "<contact id=\"65340\">.*?</contact>";
	return  my_reg(str, pattern1) && my_reg(str, pattern2);
}
...
static void show_result(char *string, char is_over)
{
	...
	prev_wakeup_flag = wakeup_flag;
	wakeup_flag = res_parse(string);
	if(prev_wakeup_flag==1 && wakeup_flag==0) {
		FILE *fp;
		fp = fopen("./SDK_result.txt", "w");
		fprintf(fp,"Result: [ %s ]", string);
		fclose(fp);
		if(is_over)
			putchar('\n');
	}
...
void on_speech_end(int reason)
{
	...
	{
		printf("\nSpeaking done \n");
		//printf("离线识别语法网络构建完成，按键 q 开始识别，按键 z 退出...\n");
	}
	else
		printf("\nRecognizer error %d\n", reason);
}
...
static void demo_mic(const char* session_begin_params)
{
	...
	/* demo 15(5) seconds recording */
	int listening_interval = 0;
	if (wakeup_flag){
		listening_interval = 5;
	}
	else {
		listening_interval = 2;
	}
	while(i++ < 5)
		sleep(1);
	errcode = sr_stop_listening(&iat);
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	while(1)
    {
		...
		/*
		if (wakeup_flag==1) {
			printf("waiting to execute functions...\n");
			ret = run_asr(&asr_data);
	        if (MSP_SUCCESS != ret) {
				printf("离线语法识别出错: %d \n", ret);
				goto exit;
