...
class PythonConfMat(caffe.Layer):
    """
    Compute the ConfMat with a Python Layer
    """

    def setup(self, bottom, top):
        # check input pair
        if len(bottom) != 2:
            raise Exception("Need two inputs to compute ConfMat.")
    ...
    def forward(self, bottom, top):
        ...
        if self.current_iter == self.test_iter:
            ...
            sys.stdout.write('\n\nConfusion Matrix')
            sys.stdout.write('\t'*(self.num_labels-2)+'|Recall')
            sys.stdout.write('\n'+'-'*8*(self.num_labels+1))
            sys.stdout.write('\n')
            for i in range(len(self.conf_matrix)):
                for j in range(len(self.conf_matrix[i])):
                    sys.stdout.write('%4d'%self.conf_matrix[i][j].astype(np.int)+'   ')
                sys.stdout.write('|%3.2f%%' % (self.conf_matrix[i][i]*100 / self.conf_matrix[i].sum()))
                sys.stdout.write('\n')
            for i in range(len(self.conf_matrix)):
                sys.stdout.write('|%3.2f%%' % (self.conf_matrix[i][i]*100 / self.conf_matrix.sum(axis=0)[i]))
            sys.stdout.write('\n')
