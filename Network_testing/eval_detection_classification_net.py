...
parser.add_argument('--net_proto_classification', type=str,
                    default='/media/pro/PRO/YH/UESTC/研究型學習/课题5_行为识别/behavior_recognition/TX2_models/Fifth_LMDB_output/SE-BN-Inception.prototxt')    # deploy.prototxt 模型
parser.add_argument('--net_weights_classification', type=str,
                    default='/media/pro/PRO/YH/UESTC/研究型學習/课题5_行为识别/behavior_recognition/TX2_models/Fifth_LMDB_output/_iter_20000.caffemodel')     # caffemodel 模型参数
...
def CaptureVideo():
    ...
    while (True):
        ...
        if len(p_box) == 0:
            ...
            p_hh_low = float(p_box[0]['p1'][1]) - (1/float((p_box[0]['p2'][1]-p_box[0]['p1'][1])))*1.0      # 记录剪裁框的位置大小
            p_hh_hige = float(p_box[0]['p2'][1]) + (1/float((p_box[0]['p2'][1]-p_box[0]['p1'][1])))*1.0
            if p_box[0]['p2'][0]-p_box[0]['p1'][0] > 400:
                p_ww_low = float(p_box[0]['p1'][0]) - (1/float((p_box[0]['p2'][0]-p_box[0]['p1'][0])))*50000.0
                p_ww_hige = float(p_box[0]['p2'][0]) + (1/float((p_box[0]['p2'][0]-p_box[0]['p1'][0])))*50000.0
            elif p_box[0]['p2'][0]-p_box[0]['p1'][0] > 300:
                p_ww_low = float(p_box[0]['p1'][0]) - (1 / float((p_box[0]['p2'][0] - p_box[0]['p1'][0]))) * 25000.0
                p_ww_hige = float(p_box[0]['p2'][0]) + (1 / float((p_box[0]['p2'][0] - p_box[0]['p1'][0]))) * 25000.0
            else:
                p_ww_low = float(p_box[0]['p1'][0]) - (1 / float((p_box[0]['p2'][0] - p_box[0]['p1'][0]))) * 10000.0
                p_ww_hige = float(p_box[0]['p2'][0]) + (1 / float((p_box[0]['p2'][0] - p_box[0]['p1'][0]))) * 10000.0
